...
Self-contained module — the rendering side of structlog config lives here so
it can be unit-tested in isolation and so ``logging/config.py`` stays focused
on the processor pipeline + uvicorn filter.
"""
...
def console_renderer(_logger: Any, _method_name: str, event_dict: dict[str, Any]) -> str:
    """Render a structlog event in fastapi-cli's ``      LEVEL   message`` format.

    Terminal renderer — must be the last processor in the chain. Mutates the
    event_dict by popping ``level`` and ``event``; remaining keys are emitted
    as ``key=value`` pairs. Control characters in both event and values are
    escaped so user-derived content can't inject terminal sequences.
    """
