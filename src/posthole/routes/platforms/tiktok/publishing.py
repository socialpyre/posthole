...
:mod:`posthole.routes.platforms.tiktok.exceptions`; the central handler converts
...
from posthole.routes.platforms.tiktok.auth import require_bearer
from posthole.routes.platforms.tiktok.exceptions import InvalidParamError, PublishNotFoundError
from posthole.routes.platforms.tiktok.responses import (
...
async def upload_chunk(
    ...
    if not content_range:
        ...
        raise InvalidParamError(msg)
    post = posts.get_by_external_ref(db, publish_id)
...
async def status_fetch(
    ...
    if post.status != "published":
        ...
        updated = posts.mark_published_by_external_ref(db, publish_id, platform_post_id)
    ...
    return tiktok_envelope(
        {
            ...
            "publicaly_available_post_id": [post.platform_post_id] if post.platform_post_id else [],
            "uploaded_bytes": 0,
            "fail_reason": "",
        }
    )
