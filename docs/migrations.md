...
1. Append a SQL string to `src/posthole/db/migrations.py::MIGRATIONS`. The
   list index _is_ the version number; don't reorder.
...
3. Each entry is run via `executescript`, so multi-statement migrations
...
- **Append-only.** Never reorder or insert a migration in the middle of the
  list. The list index is the on-disk schema version.
...
`tests/test_db.py::test_migrations_idempotent_across_reopen` is the
template.
