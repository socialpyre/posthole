...
Use GitHub's [private vulnerability reporting](https://github.com/socialpyre/posthole/security/advisories/new)
form instead. We aim to acknowledge reports within a few business days.

## Supported versions

posthole is pre-1.0 software. Only the latest released version receives security
fixes; we do not backport.

## Scope

posthole is a developer tool intended to run on a developer's local machine or in
...
- Code that allows traversal outside of the configured `POSTHOLE_DATABASE`
  directory.
...
- Findings that require modifying posthole's own code or configuration.
